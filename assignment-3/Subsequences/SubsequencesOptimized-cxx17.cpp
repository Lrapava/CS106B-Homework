 ...
 */
bool isSubsequence(std::string_view& text, std::string_view& subs);
...
bool isSubsequence(std::string_view& text, std::string_view& subs) {
	...
	std::string_view newText = text.substr(1);
	std::string_view newSubs;
	if (text[0] == subs[0]) {
...
bool isSubsequence(std::string text, std::string subs) {
	std::string_view txtview = text;
	std::string_view subview = subs;
	return isSubsequence(txtview, subview);
}
