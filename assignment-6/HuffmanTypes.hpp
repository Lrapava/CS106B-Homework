...
class Node {
	...
	Node(Node* Z, Node* O) :zero(Z), one(O) { weight = Z->weight + O->weight; }
	Node(ext_char c, int f) :character(c), zero(NULL), one(NULL), weight(f) {}

	// commented out as I am not sure how this thing is implemented
	// I was not provided instructions on how to compile the library on Linux
	// I have to start basically from scratch & focus on basic functionality.
